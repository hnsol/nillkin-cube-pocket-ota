...
### fnキーとの組み合わせはリマップ後の出力についていきます

このキーボードには、キーに水色のアイコンで刻印されたfnコンビネーションがあります。実機で確認したところ、これらは物理キーの位置ではなく、**そのキーが出力するHID usage**に結び付いています。そのため、リマップするとfnコンビネーションも一緒に移動します。

| 機能 | 工場出荷時 | `jp-lang.toml`適用後 | 確認状況 |
|---|---|---|---|
| タッチパッド／テンキー切替 | `fn` + 左 `command`（アイコン刻印あり） | `fn` + 左 `option`（左Commandを出力するキー） | 実機で確認済み |
| バッテリー残量表示 | `fn` + 右 `command`（アイコン刻印あり） | `fn` + 右 `option`（右Commandを出力するキー） | 実機で確認済み |
| スクリーンキーボード | `fn` + `control`（アイコン刻印あり） | `fn` + `caps lock`（Controlを出力するキー） | 実機で確認済み |

キーの刻印は変わらないため、アイコンが印刷されたキーと実際に機能するキーがずれます。独自のTOMLで左Command（`left_gui`）や右Command（`right_gui`）を出力するキーをなくすと、対応するfnコンビネーションを押せなくなる可能性があります（未検証）。
